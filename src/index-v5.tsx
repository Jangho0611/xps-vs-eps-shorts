...
const MarketLabels = ({top}:{top:number}) => <div style={{position:'absolute',left:66,right:66,top,display:'grid',gridTemplateColumns:'1fr 1fr',gap:24,fontFamily:PRETENDARD,fontSize:30,fontWeight:400,lineHeight:1.45,textAlign:'center',color:C.sub,pointerEvents:'none'}}><div><div>제품 예</div><div>아이소핑크 · 골드폼</div></div><div><div>제품 예</div><div>스티로폼 · 비드법 1종·2종</div></div></div>;
...
const Scene = ({n}:{n:number}) => {
  ...
  return <AbsoluteFill style={{background:C.paper,color:C.ink,fontFamily:PRETENDARD,padding:'135px 66px'}}>
    ...
    {[1,3].includes(n) && <MarketLabels top={n===1?1360:960}/>}
...
const PhotoScene = ({n}:{n:number}) => {
  ...
  return <AbsoluteFill style={{background:C.paper,color:C.ink,fontFamily:PRETENDARD,overflow:'hidden'}}>
    ...
    <div style={{position:'absolute',left:66,right:66,bottom:230,borderTop:`2px solid ${C.line}`,paddingTop:32,textAlign:'center',fontSize:51,fontWeight:700,lineHeight:1.42,whiteSpace:'pre-line'}}>{subtitles[n].join('\n')}</div>
    {[0,4].includes(n) && <MarketLabels top={1400}/>}
    <Audio src={staticFile(`assets/audio/scene0${n+1}-v2.mp3`)}/>
  </AbsoluteFill>;
};
...
registerRoot(()=><Composition id="XpsVsEpsTestV5" component={Film} width={1080} height={1920} fps={FPS} durationInFrames={BODY_FRAMES+ENDING_FRAMES}/>);
